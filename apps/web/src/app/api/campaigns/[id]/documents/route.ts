...
import { AddCampaignDocumentSchema } from '~/server/schemas/campaign.schema'
import { addCampaignDocument } from '~/server/use-cases'
...
export async function POST(
  ...
  const parsed = AddCampaignDocumentSchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const result = await addCampaignDocument(
    campaignRepository,
    campaignDocumentRepository,
    principal,
    { campaignId: idParsed.data, ...parsed.data },
  )
