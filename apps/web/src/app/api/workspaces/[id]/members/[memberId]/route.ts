...
import { UpdateMemberRoleSchema } from '~/server/schemas/workspace.schema'
...
export async function PATCH(
  ...
  if (!result.success) {
    const statusMap = {
      ...
      NOT_FOUND: 404,
      FORBIDDEN: 403,
      CANNOT_DEMOTE_LAST_OWNER: 403,
    } as const
    const messageMap = {
      UNAUTHENTICATED: 'Unauthenticated',
      NOT_FOUND: 'Workspace or member not found',
      FORBIDDEN: 'Forbidden',
      CANNOT_DEMOTE_LAST_OWNER: 'Cannot demote the only OWNER of the workspace',
    } as const
    return NextResponse.json(
      { message: messageMap[result.code] },
      { status: statusMap[result.code] },
  ...
  return NextResponse.json({ member: result.member }, { status: 200 })
}
